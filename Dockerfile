# Multi-stage build for DataMapper-on-Rust.
...
COPY src ./src
RUN cargo build --release --bin datamapper-on-rust
...
COPY --from=builder /build/target/release/datamapper-on-rust /app/datamapper-on-rust
